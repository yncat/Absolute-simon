...
<title> ランキング - Absolute Simon </title>
...
//score submission
if (isset($_POST['difficulty']) && isset($_POST['score']) && isset($_POST['player'])) {
$scores = read_scores("score_data.dat");
$_POST['player'] = str_Replace("<>", "", $_POST['player']);
$time = time(); //need this for tracking our score
array_push($scores, array($_POST['player'], $_POST['score'], $time, $_POST['difficulty']));
usort($scores, 'cmp');
$scores = array_slice($scores, 0, 1000);
//rewrite the scores
$fp = fopen("score_data.dat", "w");
if ($fp) {
flock($fp, LOCK_EX);
...
//display the ranking
if(isset($_REQUEST["dif"])){//フィルタリングパラメータ指定
$filter=$_REQUEST["dif"];
if($filter!="easy" and $filter!="normal" and $filter!="hard") $filter="";
}else{
$filter="";
}
?>
<hr>
<p>難易度で絞込み:<a href="score.php?dif=easy">easy</a> <a href="score.php?dif=normal"> normal</a> <a href="score.php?dif=hard">hard</a> <a href="score.php">解除</a></p>

<?php
echo("<table border=\"1\" cellpadding=\"5\" cellspacing=\"0\" summary=\"ランキングテーブル\">\r\n");
echo("<tr><th>順位</th><th>プレイヤー名</th><th>得点</th><th>難易度</th><th>日時</th></tr>\n");
$scores = read_scores("score_data.dat");
usort($scores, 'cmp');
$n = 1;
foreach ($scores as $score) {
